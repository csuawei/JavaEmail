...
public class ReciveOneMail {
    ...
    private String saveAttachPath = "C:\\Attachments\\"; //附件下载后的存放目录
    ...
    /**
     * 从文件名提取后缀（处理含路径、多后缀、无后缀等情况）
     * @param fileName 文件名（可含路径，如 C:\Attachments\test.pdf）
    ...
    public static String getFileSuffix(String fileName) {
        ...
        // 步骤1：去掉路径，只保留纯文件名（处理 C:\Attachments\test.pdf 这种情况）
        ...
        if (lastDotIndex == -1 || lastDotIndex == pureFileName.length() - 1) {
            return ""; // 无后缀（如 readme）或以点结尾（如 test.）
        }
        // 步骤3：返回小写后缀（统一格式，避免 PDF 和 pdf 区分）
        return pureFileName.substring(lastDotIndex + 1).toLowerCase();
    }
